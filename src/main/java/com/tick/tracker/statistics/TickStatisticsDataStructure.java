 ...
 * 
 *                              1. As Used ConcurrentSkipListMap as first() and
 *                              last() has O(1) time complexity
 * 
 *                              2. To have an optimal Memory usage I have used
 *                              the following data structure:
 *                              ConcurrentSkipListMap<Double , AtomicInteger >
 *                              where - Double is the price - AtomicInteger is
 *                              the number of occurrence, it is a counter
 *                              variable, utilizing the memory when the load is
 *                              too high.
 * 
 *                              3. This component only takes care of generating
 *                              statistics. 4. This component only takes
 *                              INSTRUMENT and PRICE, as expiry time is taken
 *                              care by reactive_stream component 5. If two
 *                              prices are sent into this component for the same
 *                              Instrument, instead of adding it in the map, it
 *                              only increment its counter.
 * 
 *                              instead of saving every price in the store, I
 *                              choose this ConcurrentSkipListMap
 * 
 *                              example of data input :
 * 
 *                              [ 1.2, 1.3, 1.1, 1.4, 1.1, 1.2, 1.4, 1.2, 1.1]
 * 
 *                              The above stream could be represented as follows
 *                              in @TickStatisticsDataStructure :
 * 
...
public class TickStatisticsDataStructure extends ConcurrentSkipListMap<Double, AtomicInteger>
	...
	public void checkin(Double value) {
		...
		sumOfActiveTicks.getAndUpdate(newValue -> {
